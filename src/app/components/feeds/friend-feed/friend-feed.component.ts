...
export class FriendFeedComponent extends Unsub implements OnInit {
  posts: Post[] = [];
  like: Like = new Like;
  userId: number = 0;
  ...
  getFriendFeed(): void {
    ...
    this.feedService.getUsersFriendFeed(this.userId).pipe(takeUntil(this.unsubscribe$)).subscribe(data => {
      this.posts = data;
